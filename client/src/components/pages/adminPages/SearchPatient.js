...
function SearchPatient() {
  ...
  async function getData() {
    ...
    try {
      const [profileResponse, patientResponse, doctorCountResponse, patientCountResponse, labCountResponse] = await Promise.all([
        fetch('http://localhost:5000/api/admin_profile', {
          headers: {
            'x-access-token': localStorage.getItem('token')
          },
        }),
        fetch('http://localhost:5000/api/display_patient', {
          headers: {
            'x-access-token': localStorage.getItem('token')
          }
        }),
      ...
      const [profileData, patientData, doctorCount, patientCount, labCount] = await Promise.all([
      ...
      if (patientData.status === 'ok') {
        setPatientData(patientData.patients)
      } else {
    ...
    <>
        ...
        <div className="container-fluid">
          <div className="row">
            ...
            <div className="col-12 links mb-5">
              ...
              <Link className="nav-link text-primary current-link" to="/searchPatient"><i className="fa-solid fa-user me-1"></i>Patient</Link>
              ...
              <Link className="nav-link text-white" to="/calendar"><i className="fa-solid fa-scroll me-1"></i>Scrawler</Link>
            </div>
      ...
      <nav className="navbar fixed-top d-navbar mb-3 shadow">
        <div className="container justify-content-start">
          <Link className="nav-link text-secondary ms-3 me-4 cur-link rounded-bottom-1" to="/searchDoctor">Search Patients</Link>
        </div>
      </nav>
              ...
              <div className="col-12" key={index}>
                <div className="col-12 d-flex">
                  <div className="col-2 pt-3 text-center">
                    <img className="search-image rounded-circle" src={patient.profileImage || process.env.PUBLIC_URL + '/images/user-solid.svg'} alt="Profile Pic" />
                  </div>
                  <div className="col-5 pt-3">
                    <h5 className="mb-2">{patient.firstName && patient.lastName
                      ? `${patient.firstName} ${patient.lastName}`
                      : patient.firstName || patient.lastName || 'Name not found'}</h5>
                    <p className="mb-0">{patient.email || 'Email not found'}</p>
                  </div>
                  <div className="col-5 pt-3 text-end">
                    <Link className="text-decoration-none edit-button" to="/editProfile" role="button"><i className="fa-solid fa-pen-to-square me-2"></i>Edit</Link>
                  </div>
                </div>
                <div className="col-12">
