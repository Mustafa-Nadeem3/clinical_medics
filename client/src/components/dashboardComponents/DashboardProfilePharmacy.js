...
function DashboardProfilePharmacist() {
  ...
  async function getProfileDetails() {
    const response = await fetch('http://localhost:5000/api/pharmacist_profile', {
      headers: {
        'x-access-token': localStorage.getItem('token'),
      },
    })
  ...
  useEffect(() => {
    ...
    if (token) {
      ...
      alert('error in p profile useEffect')
    }
  }, [])
...
export default DashboardProfilePharmacist